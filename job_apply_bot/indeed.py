...
() => {
  ...
  for (const card of cards) {
    ...
    if ((!jk && !title) || !title || title.length < 4) continue;
    ...
    if (!/(ai|ml|data|engineer|scientist|developer|architect|software|analyst)/i.test(title)) continue;
...
class IndeedDiscoveryService:
            ...
            try:
                ...
                for page_index in range(max(1, max_pages)):
                    ...
                    for candidate in candidates:
                        if len(jobs) >= max_jobs:
                            break
                        job = self._inspect_candidate(candidate=candidate, label=label, query=query, location=location, easy_apply_only=easy_apply_only)
                        if job:
                            jobs[job.job_id] = job
    ...
    def _inspect_candidate(self, candidate: Dict[str, str], label: str, query: str, location: str, easy_apply_only: bool) -> Optional[JobRecord]:
        ...
        if not self._matches_experience_constraints(title, card_text):
            return None

        posted_at = self._parse_posted_at(candidate.get("postedText", ""))
        ...
        return JobRecord(
            ...
            status="pending",
            reason=self._default_reason(ai_ml_match, posted_at, easy_apply, easy_apply_only),
        )
    ...
    def _default_reason(self, ai_ml_match, posted_at, easy_apply: bool, easy_apply_only: bool) -> str:
        if not ai_ml_match:
